...
class SystemInspector:
    ...
    def extrema(self, trajectory=None):
        rows = []
        for i in self._selected_trajectories(trajectory):
            solution = self.solution(i)
            times = self.time(i)
            for axis, col in AXES.items():
                values = solution[:, col]
                for kind, indices in [
                    ("min", self._local_minima_indices(values)),
                    ("max", self._local_maxima_indices(values)),
                ]:
        ...
        columns = ["trajectory", "axis", "kind", "step", "t", "value"]
    ...
    def vector_field_slice(
        ...
        *,
        ...
        x_values = np.linspace(float(x_range[0]), float(x_range[1]), density)
        y_values = np.linspace(float(y_range[0]), float(y_range[1]), density)
    ...
    def unfollow(self, plot=None):
        return self._window.live_plot_controller._remove_follow(plot)

    def unfollow_all(self):
        return self._window.live_plot_controller._clear_follow()
    ...
    def _unique_return_pairs(self, frame):
        if frame.empty:
            return frame

        pairs = np.sort(frame[["step", "match_step"]].to_numpy(), axis=1)
        keep = ~pd.MultiIndex.from_arrays([pairs[:, 0], pairs[:, 1]]).duplicated()

        return frame.loc[keep].reset_index(drop=True)

    def _plot_timeseries_frame(
        self,
        frame,
        value_column,
        *,
    ...
    def _follow_live_plot(self, kind, *, plot=None, **options):
        mode = self._plot_mode(options.pop("mode", PLOT_MODE_REPLACE))
        registration = self._window.live_plot_controller._set_follow(
            kind,
            plot=plot,
            mode=mode,
            **options,
    ...
    def _crossing_trajectories(self, trajectory, *, t_max=None, n=None):
        if t_max is None and n is None:
            for i in self._selected_trajectories(trajectory):
                yield i, self.solution(i), self.time(i)
            return
    ...
    def _local_maxima_indices(self, values):
        if len(values) < 3:
    ...
    def _local_minima_indices(self, values):
        if len(values) < 3:
            return np.array([], dtype=int)
